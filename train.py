...
dataset_module = importlib.import_module('_'.join(['dataset', opts.experiment]))  # imports dataset_xyz.py

# Initialise an object of type xyzDataset, which can be used as input to DataLoader. For synthetic, it has members
# .dataset, which contains datasets for each distribution type and is of dimension (4*2500, 200, 1), .targets of size
# (1, 4*2500) which contains the corresponding targets, and .means and .variances which are of size (1, 4*2500) each and
# contain the distribution means and variances respectively.
...
test_dataset = dataset_module.get_dataset(opts, split='val')  # Split can be train, val or test
...
for epoch in tqdm.tqdm(range(opts.num_epochs)):
    ...
    for data_dict in train_dataloader:
        ...
        # Make a forward pass through model. The output is a dictionary with (example dims for synthetic):
        ...
        # TODO: Check here that computation works for youtube data
